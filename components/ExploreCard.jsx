...
const ExploreCard = ({ id, imgUrl, title, description, index, active, handleClick }) => (
  ...
  >
    <img src={basePath + imgUrl} alt={title} className="absolute w-full h-full object-cover rounded-[24px]" />
    ...
    { active !== id ? (
      ...
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        <div className={`${styles.flexCenter} w-[60px] h-[60px] glassmorphism mb-[16px] rounded-[12px]`}>
          <img src={`${basePath}/headset.svg`} alt="headset" className="w-1/2 h-1/2 object-cotain" />
        </div>
        {/* <p className="font-normal text-[16px] leading-[20.16px] text-white uppercase">Enter Metaverse</p> */}
        <h2 className="mt-[24px] font-semibold sm:text-[32px] text-[24px] text-white">
          {title}
        </h2>
        <p className="mt-[16px] font-normal text-[16px] leading-[20.16px] text-white">
          {description}
        </p>
      </div>
    )}
